...
%for CurrentSlice = 1:10;    
    ...
    for CurrentProtocol = 1:length(Protocols)
        ...
        figure;
            subplot(221)
                imshow(Details(CurrentProtocol).Slice)
                title([Protocols(CurrentProtocol) ' - original'])
            subplot(222)
                imshow(Details(CurrentProtocol).ResizedSlice)
                title([Protocols(CurrentProtocol) ' - resized'])
            if doThreshold == 1
                subplot(223)
                    imshow(Details(CurrentProtocol).ThresholdedSlice)
                    title([Protocols(CurrentProtocol) ' - thresholded'])
            end
            subplot(224)
                imshow(Details(CurrentProtocol).DifferenceImage)
                title([Protocols(CurrentProtocol) ' - difference'])
    end
    close(w);
...
if isunix == 0
    OutputPath = 'P:\MATLAB\WideFieldScan\Paper\NormErrorOutput';
    [status,message,messageid] = mkdir(OutputPath);
...
figure
    ...
    title(['Mean Normalized "Quality" (au) of the Difference Image compared to Protocol "b" for ' num2str(SliceCounter-1) ' Slices'])
    set(gca,'XTick',[1:length(Protocols)])
    set(gca,'XTickLabel',rot90(Protocols))
if isunix == 0
    matlab2tikz('2008cMeanNormalizedCumulativeErrorPlot.tex')
end    

figure
    plot(mean(NormCumulativeError))
    title(['Mean Normalized Error per Pixel (au) of the Difference Image compared to Protocol "b" for ' num2str(SliceCounter-1) ' Slices'])
    set(gca,'XTick',[1:length(Protocols)])
    set(gca,'XTickLabel',rot90(fliplr(Protocols)))
if isunix == 0
...
if isunix == 0
    disp('Use 2008cMeanNormalizedCumulativeErrorPlot.tex for the plot in the paper!');
	disp('Since matlab2tikz 0.0.4, errorplots work nicely');
    disp('The x-axis goes from 1:19 for each protocol, maybe you need to adapt it to the TotalScannignTime');
end
