...
# This is the flag used for Testing Purposes
# Set to `testing = 0` for an actual scan!
testing = 0
...
EPICS_ROI=EpicsChannel("X02DA-SCAN-CAM1:ROI")
...
wasROI=EPICS_ROI.getValCHK(EPICS_ROI.connected)

parameterfile = open(filename)
counter = 1

print "reading SampleName from Panel"
curroi = EPICS_ROI.getValCHK(EPICS_ROI.connected)
if testing == 0:
...
while 1:
  ...
  # set filename
  if curroi == "" :
    SubScanName = "s" + str(counter)
    print "AAA: '" + curroi + "'"  + SubScanName + "'"
  else:
    SubScanName = curroi + "_s" + str(counter)
    print "BBB: '" + curroi + "'->'" + SubScanName + "'"
  print "The Suffix for the current SubScan is " + SubScanName
  
  #Start scanning
  EPICS_ROI.putValCHK(SubScanName,EPICS_ROI.connected)
  print "I've set the ROI-name for the current SubScan on the EPICS-Panel"
  ...
  while scanstatus == 1:
    ...
    if testing == 0:
      ...
      print "performing fake test-scan scan nr. " + str(31-testcounter)
...
EPICS_ROI.putValCHK(wasROI,EPICS_InBeamPosition.connected)
print
print
