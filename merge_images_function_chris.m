function merge_images_function_chris(templatesize,DetectorSize,Overlap,NumRows,StepWidth,StepRange,SignalToNoiseRate)
  testslice = phantom('Modified Shepp-Logan',templatesize);% ,'gaussian',0.0,10^(-SignalToNoiseRate/2) / 6) ;
%  max(max(testslice))
%  figure(1);
%  imshow(testslice,[]);
%  for slice = 1:128;
...
%    slices(block).slicedata =  - log ( ( slices(block).slicedata ) * 0.6  + 0.3  );
    slices(block).slicedata = imnoise(slices(block).slicedata,'gaussian',0.0,10^(-SignalToNoiseRate/2)/6 );
